...
public:
  ...
  /**
   * void applyForce(Vector3D force) method is used to add external 
   * force to the mass. At an instance in time, several sources of
   * force might affect the mass. The vector sum of these forces make 
   * up the net force applied to the mass at the instance.
   */
  void applyForce(Vector3D force)
  {
    this->force += force; // The external force is added to the force
                          // of the mass
  }
